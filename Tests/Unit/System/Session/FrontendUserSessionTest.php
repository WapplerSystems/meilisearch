...
class FrontendUserSessionTest extends SetUpUnitTestCase
{
    ...
    public function sessionDataWillBeRetrievedFromSessionForLastSearches(): void
    {
        ...
        $this->feUserMock->expects(self::once())->method('getKey')->with('ses', 'tx_t3meilisearch_lastSearches')->willReturn($fakeSessionData);
    ...
    public function canSetLastSearchesInSession(): void
    {
        ...
        $this->feUserMock->expects(self::once())->method('setKey')->with('ses', 'tx_t3meilisearch_lastSearches', $lastSearches);
    ...
    {
        $fakeSessionData = 12;
        $this->feUserMock->expects(self::once())->method('getKey')->with('ses', 'tx_t3meilisearch_resultsPerPage')->willReturn($fakeSessionData);
    ...
    {
        $lastSearches = 45;
        $this->feUserMock->expects(self::once())->method('setKey')->with('ses', 'tx_t3meilisearch_resultsPerPage', $lastSearches);
