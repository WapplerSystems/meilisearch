...
class SearchUriBuilderTest extends SetUpUnitTestCase
{
    ...
    public function addFacetLinkIsCalledWithSubstitutedArguments(): void
    {
        ...
        $configurationMock->expects(self::any())->method('getSearchPluginNamespace')->willReturn('tx_t3meilisearch');
        ...
        $expectedArguments = ['tx_t3meilisearch' => ['filter' => ['###tx_t3meilisearch:filter:0:foo###']]];
    ...
    {
        $expectedArguments = ['tx_t3meilisearch' => ['filter' => ['###tx_t3meilisearch:filter:0:option###']], 'foo' => '###foo###'];
        $linkBuilderResult = '/index.php?id=1&filter=' . urlencode('###tx_t3meilisearch:filter:0:option###') . '&foo=' . urlencode('###foo###');
        ...
        $this->routingServiceMock->expects(self::any())
            ...
            ->willReturn(['###tx_t3meilisearch:filter:0:option###' => 'option%3Avalue', '###foo###' => 'bar']);
        $configurationMock->expects(self::once())
    ...
    public function setArgumentsIsOnlyCalledOnceEvenWhenMultipleFacetsGetRendered(): void
    {
        ...
        $expectedArguments = ['tx_t3meilisearch' => ['filter' => ['###tx_t3meilisearch:filter:0:color###']]];
        $this->extBaseUriBuilderMock->expects(self::once())->method('setArguments')->with($expectedArguments)->willReturn($this->extBaseUriBuilderMock);
        $this->extBaseUriBuilderMock->expects(self::once())->method('reset')->with()->willReturn($this->extBaseUriBuilderMock);
        $this->extBaseUriBuilderMock->expects(self::once())->method('build')->willReturn(urlencode('/index.php?id=1&tx_t3meilisearch[filter][0]=###tx_t3meilisearch:filter:0###'));
    ...
    {
        $expectedArguments = ['tx_t3meilisearch' => ['sort' => '###tx_t3meilisearch:sort###']];
        $linkBuilderResult = '/index.php?id=1&' . urlencode('tx_t3meilisearch[sort]') . '=' . urlencode('###tx_t3meilisearch:sort###');
        ...
        $this->routingServiceMock->expects(self::any())
            ->method('reviseFilterVariables')
            ->willReturn(['###tx_t3meilisearch:sort###' => 'title+desc']);
        ...
        self::assertEquals('/index.php?id=1&' . urlencode('tx_t3meilisearch[sort]') . '=' . urlencode('title desc'), $result);
    }
    ...
    public function canGetRemoveFacetOptionUri(): void
    {
        ...
        $expectedArguments = ['tx_t3meilisearch' => ['filter' => []]];
        $this->extBaseUriBuilderMock->expects(self::once())->method('setArguments')->with($expectedArguments)->willReturn($this->extBaseUriBuilderMock);
        $this->extBaseUriBuilderMock->expects(self::once())->method('reset')->with()->willReturn($this->extBaseUriBuilderMock);
    ...
    public function canGetRemoveFacetUri(): void
    {
        ...
        // we expect that the filters are empty after remove
        //@todo we need to refactor the request in EXT:t3_meilisearch to cleanup empty arguments completely to assert  $expectedArguments = []
        $expectedArguments = ['tx_t3meilisearch' => ['filter' => []]];
        $this->extBaseUriBuilderMock->expects(self::once())->method('setArguments')->with($expectedArguments)->willReturn($this->extBaseUriBuilderMock);
        $this->extBaseUriBuilderMock->expects(self::once())->method('reset')->with()->willReturn($this->extBaseUriBuilderMock);
    ...
    public function addFacetUriRemovesPreviousGroupPage(): void
    {
        ...
        $this->extBaseUriBuilderMock->expects(self::once())->method('reset')->with()->willReturn($this->extBaseUriBuilderMock);
        $this->extBaseUriBuilderMock->expects(self::any())->method('build')->willReturn('/index.php?id=1&tx_t3meilisearch[filter][0]=type:pages');

        $uri = $this->searchUrlBuilder->getAddFacetValueUri($previousRequest, 'type', 'pages');
        self::assertSame('/index.php?id=1&tx_t3meilisearch[filter][0]=type:pages', urldecode($uri), 'Unexpected uri generated');
    }
    ...
    public function canSetGroupPageForQueryGroup(): void
    {
        $expectedArguments = [
            'tx_t3meilisearch' => [
                'groupPage' => [
                    'smallPidRange' => [
                        'pid0to5' => '###tx_t3meilisearch:groupPage:smallPidRange:pid0to5###',
                    ],
                ],
            ],
        ];
        ...
        $this->routingServiceMock->expects(self::any())
            ->method('reviseFilterVariables')
            ->willReturn(['###tx_t3meilisearch:groupPage:smallPidRange:pid0to5###' => '5']);
        $uri = $this->searchUrlBuilder->getResultGroupItemPageUri($previousRequest, $groupItem, 5);
        self::assertStringContainsString(urlencode('tx_t3meilisearch[groupPage][smallPidRange][pid0to5]') . '=5', $uri, 'Uri did not contain link segment for query group');
    }
    ...
    public function siteConfigurationModifyUriTest(): void
    {
        ...
        $expectedQueryParameters = [
            'tx_t3meilisearch' => [
                'filter' => [
                    '###tx_t3meilisearch:filter:0:type###',
                    '###tx_t3meilisearch:filter:1:color###',
                    '###tx_t3meilisearch:filter:2:color###',
                    '###tx_t3meilisearch:filter:3:color###',
                    '###tx_t3meilisearch:filter:4:taste###',
                    '###tx_t3meilisearch:filter:5:taste###',
                    '###tx_t3meilisearch:filter:6:product###',
                    '###tx_t3meilisearch:filter:7:product###',
                ],
            ],
        ];
        ...
        $expectedQueryParameters = [
            'tx_t3meilisearch' => [
                'filter' => [
                    ...
                    '###tx_t3meilisearch:filter:6:product###',
                    '###tx_t3meilisearch:filter:7:product###',
                    '###tx_t3meilisearch:filter:8:quantity###',
                ],
            ],
        ];
        $linkBuilderResult = '/index.php?id=42&color=' . urlencode('green,red,yellow') .
            '&taste=' . urlencode('matcha,sour') .
            '&product=' . urlencode('candy,sweets') .
            '&' . urlencode('tx_t3meilisearch[filter][0]') . '=' . urlencode('quantity:20');
        $configurationMock = $this->createMock(TypoScriptConfiguration::class);
        $configurationMock->expects(self::any())->method('getSearchPluginNamespace')->willReturn('tx_t3meilisearch');
        $configurationMock->expects(self::once())->method('getSearchTargetPage')->willReturn(42);
