...
{
    /**
     * The item's uid in the index queue (tx_t3meilisearch_indexqueue_item.uid)
     */
    protected ?int $indexQueueUid = null;

    /**
     * The root page uid of the tree the item is located in (tx_t3meilisearch_indexqueue_item.root)
     */
    protected ?int $rootPageUid;

    /**
     * The record's type, usually a table name, but could also be a file type (tx_t3meilisearch_indexqueue_item.item_type)
     */
    protected ?string $type;

    /**
     * The name of the indexing configuration that should be used when indexing (tx_t3meilisearch_indexqueue_item.indexing_configuration)
     * the item.
     */
    protected string $indexingConfigurationName;

    /**
     * The unix timestamp when the record was last changed (tx_t3meilisearch_indexqueue_item.changed)
     */
    protected ?int $changed = null;

    /**
     * The unix timestamp when the record was last indexed (tx_t3meilisearch_indexqueue_item.indexed)
     */
     ...
     *
     * @param array $itemMetaData Metadata describing the item to index using the index queue. Is expected to contain a record from table tx_t3meilisearch_indexqueue_item
